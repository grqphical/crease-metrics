...
func CreateLeaderboardProxyHandler(key string) func(*gin.Context) {
...
func IndividualStatsHandler(c *gin.Context) {
	state, _ := c.Get("appState")
	db := state.(*APIState).db
	...
	if err == ErrInvalidCache {
		resp, err := http.DefaultClient.Get(fmt.Sprintf("https://api-web.nhle.com/v1/player/%s/landing", id))
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
